...
export class ScaffoldCommand {
  private validateChangeId(changeId: string): void {
    const kebabCaseRegex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;

    if (!kebabCaseRegex.test(changeId)) {
      throw new Error(
        `Invalid change ID "${changeId}". Must be kebab-case (lowercase letters, numbers, and hyphens only).`
      );
    }
  }

  private validateCapabilityName(capability: string): void {
    const kebabCaseRegex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;

    if (!kebabCaseRegex.test(capability)) {
      throw new Error(
        `Invalid capability name "${capability}". Must be kebab-case (lowercase letters, numbers, and hyphens only).`
      );
    }
  }

  private parseCapabilityNames(input: string): string[] {
    if (!input || input.trim() === '') {
      return [];
    }
  ...
  private generateSpecTemplate(capability: string): string {
